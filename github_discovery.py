...
def create_summary(
  services, processed_components, processed_products, duplicate_appinsights_cloud_role, force_update=False
):
  ...
  def summarize_duplicate_app_role_with_details(duplicate_roles, item_type, force_update=False):  
    """
    Summarizes duplicate Application Insights Cloud Role Names with details of components.
    """
    if force_update:
        summary = f'\n\n{item_type.upper()} SUMMARY\n{"=" * (len(item_type) + 8)}\n'
    else:
    ...
    summary += f'{len(duplicate_roles)} {item_type.lower()}(s) found with duplicate Application Insights Cloud Role Name\n'
  ...
  summary += summarize_duplicate_app_role_with_details(duplicate_appinsights_cloud_role, 'component', force_update )
  # summary += summarize_processed_items(processed_teams, 'team', team_attributes)
  summary += '\n_(generated by <https://github.com/ministryofjustice/hmpps-github-discovery|hmpps-github-discovery>)_'
...
def main():
  ...
  create_summary(services, processed_components, processed_products, duplicate_appinsights_cloud_role, force_update)
