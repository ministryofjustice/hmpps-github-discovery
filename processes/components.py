...
def get_app_insights_cloud_role_name(
  ...
  languages = {key: value for key, value in languages.items() if isinstance(value, int)}
...
def update_app_type(component, data):
  ...
  if found_topics:
    # Topic-driven detection: only enable flags explicitly mentioned
    # and leave existing values for other types unchanged
